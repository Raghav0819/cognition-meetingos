...
def get_crew_tasks(meeting_id: str, transcript: str, participants: list):
    ...
    summarize = Task(
        ...
        You have access to the original transcript and all the processed task data from previous agents.
        ...
        agent=summary_agent,
        context=[followup, extract]
    )

    audit = Task(
        description=f"""
        This is the final step. Take all tasks from the previous agents and the MoM summary and:
        ...
        3. Use the SaveLogTool to log: meeting_id="{meeting_id}", 
           ...
           action="CrewAI completed processing. All tasks saved. MoM generated."
        ...
        4. Return a final JSON object:
           {{
             ...
             "mom": <the complete Minutes of Meeting text from the summary agent>,
             "status": "completed"
           }}

        IMPORTANT: The "mom" field must contain the FULL Minutes of Meeting text from the summary agent.
        Do not truncate or summarize it. Include every section exactly as written.
        """,
        expected_output="Final JSON with meeting_id, total_tasks, tasks array, validation_summary, mom, status",
        agent=audit_agent,
        context=[followup, summarize]
    )

    return [extract, assign, score, validate, followup, summarize, audit], [
        extraction_agent, assignment_agent, confidence_agent,
        validation_agent, followup_agent, summary_agent, audit_agent
    ]